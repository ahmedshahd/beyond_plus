...
function convertGovernorateId(areas) {
  const governorateIds = {};
  let currentId = 28;

  const convertedAreas = areas.map((area) => {
    const { governorate_id, ...rest } = area;

    if (!governorateIds.hasOwnProperty(governorate_id)) {
      governorateIds[governorate_id] = currentId.toString();
      currentId++;
    }

    return {
      ...rest,
      governorate_id: governorateIds[governorate_id],
    };
  });
  console.log('convertedAreas', convertedAreas);
}

async function main() {
  convertGovernorateId(areas);

  //   console.log(`Start seeding ...`);
  //   const convertedAreas = areas.map((area, index) => {
  //     return {
  //       ...area,
  //       governorate_id: (28 + index).toString(),
  //     };
  //   });
  //   for (const area of areas) {
  //     const user = await prisma.clientArea.create({
  //       data: {
  //         name: area.city_name_en,
  //         // clientCityId: parseInt(convertedAreas.governorate_id),
  //         language: 'ENGLISH',
  //       },
  //     });
  //     console.log(`Created user with id: ${area.city_name_en}`);
  //   }
