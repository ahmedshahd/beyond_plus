...
async function main() {
  ...
  for (const city of cities) {
    ...
    const createdCity = await prisma.clientCity.create({
      data: {
        id: i,
        name: city.governorate_name_ar,
        language: 'ARABIC',
      },
    });
    i = i + 1;
    console.log(`Created user with id: ${createdCity.id}`);

    console.log(`Created user with id: ${city.governorate_name_ar}`);
  }
